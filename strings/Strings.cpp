...
void Strings::fillArr(char alpha[], int dimAlpha){
    for (int i = 0; i < dimAlpha; ++i) {
        alpha[i] = i + 97;
    }
}

void Strings::convertStr(char str[], int size){
    int first = str[0];
    ...
    std::cout<< "La stringa convertita --> %s"<<str;
}

int Strings::strlength(char str[]){
    int len = 0;
...
void Strings::countChar(char structure[], char str[], int len){
    int charCount[26];
    int i, j;
...
void Strings::compareLen(char str1[], char str2[]){
...
void Strings::strInvert(char str[]){
    for (int i = (strlength(str)-1); i >= 0; i--){
        (str[i] != '/0') && printf("%c", str[i]);
    }
}
void Strings::decToBin(char num[]){
    int number = atoi(num);
    char strBin[20];
    int i = 0;
